...
def generate_trees_on_terrace(out, triplet_file, messages=sys.stderr):
    ...
    subparents = sample(parents, 50)
    sublist = TreeList(subparents, taxon_namespace=tns)
    num_par = len(parents)
    sub_par = len(subparents)
    ...
    sublist.write_to_stream(dest=out, schema='newick', suppress_edge_lengths=True, suppress_internal_node_labels=True, suppress_internal_taxon_labels=True, suppress_annotations=True, unquoted_underscores=True)
    ...
    if messages:
        messages.write('sampled %d trees out of %d trees on terrace\n' % (sub_par, num_par))
...
class RandomSelectionNode(object):
    ...
    def choose():
        if here:
            ...
            left = choice((left_node).choose())
            right = choice((right_node).choose())

            return combine_subtrees([left], [right])


def sample_trees_on_terrace(out, triplet_file, messages=sys.stderr):
    label_set, triplets = read_triplet_file(triplet_file, messages=messages)

    tns =  TaxonNamespace(label_set)
    
    messages.write('Sampling  parent treddes on terrace...\n')

    #base_node = RandomSelectionNode()
    #superb_generate_master_tree(base_node, label_set, triplets, tns)
    superb_generate_master_treeis(label_set, triplets, tns)

    #parent = base_node.choose()
...
def superb_generate_master_tree(sample_node, label_set, triplets, tns):
    '''SUPERB algorithm of Constantinescu and Sankoff, 1995, to generate parent trees
    compatible with given set of triplets  
    
    There are several things that can happen as we recurse through the tree:
    -we reach a point where we have a label_set but no relevant triplets
        -generate all subtrees for that label_set
    -we reach a point where we have multiple components:
        - the components are divided into two sets, i.e. the left and right subtrees
          tree objects for each possible subtree are created by:
            -each of those sets (components, ~subtrees) can consist of
            -a leaf (tree is just a leaf)
            -a pair of leaves (tree is a cherry)
            -more leaves (recursive call to this function, returning a set of
             "alternative" subtrees
        -the collected left and right subtrres are combined combinatorically
         to creat all possible subtrres at the present node
    ...
    if not triplets:
        ...
        if num_components > 1:
            ...
            left  = RandomSelectionNode()
            right = RandomSelectionNode()
            ...
            for i in xrange(1, num_biparts + 1):
                ...
                if len(subset1) == 1:
                    ...
                    #q = superb(subset1, new_triplets)
                    #left_subtrees.extend(superb_generate_trees(subset1, new_triplets, tns))
                    left_subtrees._trees.extend(superb_generate_master_tree(left, subset1, new_triplets, tns)._trees)
                ...
                if len(subset2) == 1:
                    ...
                    #v = superb(subset2, new_triplets)
                    #right_subtrees.extend(superb_generate_trees(subset2, new_triplets, tns))
                    right_subtrees._trees.extend(superb_generate_master_tree(right, subset2, new_triplets, tns)._trees)


                sample_node.left_node = left
                sample_node.right_node = right
