...
# USAGE: treesequal.pl file

# where file is a file having two newick trees in it.
...
### die "Must supply exactly two input trees\n" if ($ntree != 2);


# ISN'T THE FOLLOWING REDUNDANT AND STUPID????
foreach $nwk (@nwks)
	{
	$t = treeInit($nwk);
	}
